...
// UserProofRunnerConfig is the T2 model-typed dependency bundle for one
// userproof run.
...
func sortedKeys(m map[int][]t2spec.AccountInfo) []int {
...
// BuildUserProofRow serialises one T2 account into a UserProof DB row.
func BuildUserProofRow(
	account *t2spec.AccountInfo,
	leaf []byte,
	proof [][]byte,
	rootHex string,
