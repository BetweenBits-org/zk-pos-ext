...
func EncodeBatchWitness(w *t2spec.BatchCreateUserWitness) ([]byte, error) {
...
func DecodeBatchWitness(data []byte) (*t2spec.BatchCreateUserWitness, error) {
	uncompressed, err := s2.Decode(nil, data)
	if err != nil {
		return nil, fmt.Errorf("decompress batch witness: %w", err)
	}
	var w t2spec.BatchCreateUserWitness
	...
	for i := range w.CreateUserOps {
		dense := make([]t2spec.AccountAsset, capacity)
		for p := range capacity {
			dense[p] = t2spec.AccountAsset{Index: uint16(p)}
		}
