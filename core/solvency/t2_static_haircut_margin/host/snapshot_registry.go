...
// RegisterSnapshot adds a T2 snapshot connector factory. Panics on an
// empty id, nil factory, or duplicate registration.
func RegisterSnapshot(connectorID string, factory SnapshotFactory) {
	if connectorID == "" {
		panic("t2_static_haircut_margin/host: RegisterSnapshot called with empty connector id")
	}
	if factory == nil {
		panic("t2_static_haircut_margin/host: RegisterSnapshot(" + connectorID + ") called with nil factory")
	}
	snapshotRegistryMu.Lock()
	defer snapshotRegistryMu.Unlock()
	if _, dup := snapshotRegistry[connectorID]; dup {
		panic("t2_static_haircut_margin/host: snapshot connector " + connectorID + " registered twice")
	}
