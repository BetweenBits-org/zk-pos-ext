...
// WitnessRunnerConfig is the T2 model-typed dependency bundle. T2
// adds a single collateral pool per asset on top of T1's
// equity/debt; static haircut_bp lives on CexAssetInfo and does not
// move between batches.
type WitnessRunnerConfig struct {
	Ctx             context.Context
	Snapshot        t2spec.SnapshotSource
...
// docstring; the T2 variant is identical at the wiring layer.
...
func streamAndBucket(ctx context.Context, snapshot t2spec.SnapshotSource, tiers []int) (map[int][]t2spec.AccountInfo, error) {
	ch, err := snapshot.AccountStream(ctx)
	if err != nil {
		return nil, fmt.Errorf("AccountStream: %w", err)
	}
	out := make(map[int][]t2spec.AccountInfo)
	for account := range ch {
		tier := t2spec.PickAssetCountTier(t2spec.CountNonEmptyAssets(account.Assets), tiers)
		if tier == 0 {
			return nil, fmt.Errorf("account %d has %d non-empty assets — no tier in %v fits",
				account.AccountIndex, t2spec.CountNonEmptyAssets(account.Assets), tiers)
		}
...
func runBatches(
	accountsByTier map[int][]t2spec.AccountInfo,
	cexAssets []t2spec.CexAssetInfo,
	accountTree bsmt.SparseMerkleTree,
	witnessStore *store.WitnessStore,
	assetCountTiers []int,
	shapeProvider corespec.BatchShapeProvider,
...
func buildBatch(
	batch []t2spec.AccountInfo,
	cexAssets []t2spec.CexAssetInfo,
	accountTree bsmt.SparseMerkleTree,
	assetCountTiers []int,
) (*t2spec.BatchCreateUserWitness, error) {
	wit := &t2spec.BatchCreateUserWitness{
		BeforeAccountTreeRoot: accountTree.Root(),
		BeforeCexAssets:       make([]t2spec.CexAssetInfo, len(cexAssets)),
		CreateUserOps:         make([]t2spec.CreateUserOperation, len(batch)),
	}
	...
	for i, account := range batch {
		...
		// T2: equity/debt + single collateral pool. Haircut is static
		// per-asset metadata, untouched per batch.
