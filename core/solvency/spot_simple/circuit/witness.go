...
//
// Compared to tier_3bucket's SetBatchCreateUserCircuitWitness: no
...
func SetBatchCreateUserCircuitWitness(
	batchWitness *spotspec.BatchCreateUserWitness,
	assetCountTiers []int,
	...
	targetCounts := spotspec.PickAssetCountTier(
		spotspec.CountNonEmptyAssets(batchWitness.CreateUserOps[0].Assets),
		assetCountTiers,
	)
	for i := range w.CreateUserOps {
		...
		for j := range batchWitness.CreateUserOps[i].Assets {
			...
			if !spotspec.IsAccountAssetEmpty(&u) {
		...
		// tier_3bucket fix in commit d7c23f3).
