...
// Same wire shape as tier_3bucket/host.EncodeBatchWitness — gob + s2.
func EncodeBatchWitness(w *spotspec.BatchCreateUserWitness) ([]byte, error) {
...
// keygen time. Mirrors tier_3bucket/host.DecodeBatchWitness; the
// per-asset 1-tuple shape is the model-specific difference.
func DecodeBatchWitness(data []byte) (*spotspec.BatchCreateUserWitness, error) {
	uncompressed, err := s2.Decode(nil, data)
	if err != nil {
		return nil, fmt.Errorf("decompress batch witness: %w", err)
	}
	var w spotspec.BatchCreateUserWitness
	...
	for i := range w.CreateUserOps {
		dense := make([]spotspec.AccountAsset, capacity)
		for p := range capacity {
			dense[p] = spotspec.AccountAsset{Index: uint16(p)}
		}
