...
func EncodeBatchWitness(w *t3spec.BatchCreateUserWitness) ([]byte, error) {
...
func DecodeBatchWitness(data []byte) (*t3spec.BatchCreateUserWitness, error) {
	uncompressed, err := s2.Decode(nil, data)
	if err != nil {
		return nil, fmt.Errorf("decompress batch witness: %w", err)
	}
	var w t3spec.BatchCreateUserWitness
	...
	for i := range w.CreateUserOps {
		dense := make([]t3spec.AccountAsset, capacity)
		for p := range capacity {
			dense[p] = t3spec.AccountAsset{Index: uint16(p)}
		}
