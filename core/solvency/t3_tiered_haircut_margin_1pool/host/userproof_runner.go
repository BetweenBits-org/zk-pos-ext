...
// UserProofRunnerConfig is the T3 model-typed dependency bundle for one
// userproof run.
...
func sortedKeys(m map[int][]t3spec.AccountInfo) []int {
...
// BuildUserProofRow serialises one T3 account into a UserProof DB row.
func BuildUserProofRow(
	account *t3spec.AccountInfo,
	leaf []byte,
	proof [][]byte,
	rootHex string,
