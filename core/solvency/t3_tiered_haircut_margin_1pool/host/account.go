...
func AccountLeafHash(account *t3spec.AccountInfo, assetCountTiers []int) []byte {
...
func PaddingAccounts(
	accounts []t3spec.AccountInfo,
	assetKey int,
	paddingStartIndex int,
	usersPerBatch int,
) (int, []t3spec.AccountInfo) {
	...
	for range paddingAccountCounts {
		assets := make([]t3spec.AccountAsset, assetKey)
		for j := range assetKey {
			assets[j] = t3spec.AccountAsset{Index: uint16(j)}
		}
		accounts = append(accounts, t3spec.AccountInfo{
