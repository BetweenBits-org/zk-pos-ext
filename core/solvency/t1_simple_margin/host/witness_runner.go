...
// WitnessRunnerConfig is the T1 model-typed dependency bundle for a
// witness run. Same shape as the T4 runner — only the SnapshotSource
// type and the buildBatch inner loop differ (T1 has no collateral).
...
// docstring; the T1 variant is identical at the wiring layer.
...
func streamAndBucket(ctx context.Context, snapshot t1spec.SnapshotSource, tiers []int) (map[int][]t1spec.AccountInfo, error) {
	ch, err := snapshot.AccountStream(ctx)
	if err != nil {
		return nil, fmt.Errorf("AccountStream: %w", err)
	}
	out := make(map[int][]t1spec.AccountInfo)
	for account := range ch {
		tier := t1spec.PickAssetCountTier(t1spec.CountNonEmptyAssets(account.Assets), tiers)
		if tier == 0 {
			return nil, fmt.Errorf("account %d has %d non-empty assets — no tier in %v fits",
				account.AccountIndex, t1spec.CountNonEmptyAssets(account.Assets), tiers)
		}
...
func buildBatch(
	batch []t1spec.AccountInfo,
	cexAssets []t1spec.CexAssetInfo,
	accountTree bsmt.SparseMerkleTree,
	assetCountTiers []int,
) (*t1spec.BatchCreateUserWitness, error) {
	wit := &t1spec.BatchCreateUserWitness{
		BeforeAccountTreeRoot: accountTree.Root(),
		BeforeCexAssets:       make([]t1spec.CexAssetInfo, len(cexAssets)),
		CreateUserOps:         make([]t1spec.CreateUserOperation, len(batch)),
	}
	...
	for i, account := range batch {
		...
		// T1 has no collateral — only equity/debt accumulate per asset.
