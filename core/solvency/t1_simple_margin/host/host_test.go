...
func TestEncodeDecodeBatchWitness_RoundTrip(t *testing.T) {
	const capacity = 8
	beforeCex := make([]t1spec.CexAssetInfo, capacity)
	beforeCex[0] = t1spec.CexAssetInfo{
		TotalEquity: 100, BasePrice: 7_000_000, Symbol: "BTC", Index: 0,
	}
	src := &t1spec.BatchCreateUserWitness{
		...
		CreateUserOps: []t1spec.CreateUserOperation{
			{
	...
	encoded, err := t1host.EncodeBatchWitness(src)
	if err != nil {
		t.Fatalf("encode: %v", err)
	}
	dst, err := t1host.DecodeBatchWitness(encoded)
	if err != nil {
...
func TestComputeCexAssetsCommitment_Deterministic(t *testing.T) {
	...
	const capacity = 5
	h1 := t1host.ComputeCexAssetsCommitment(assets, capacity)
	h2 := t1host.ComputeCexAssetsCommitment(assets, capacity)
	...
	swapped := []t1spec.CexAssetInfo{assets[1], assets[0]}
	hSwap := t1host.ComputeCexAssetsCommitment(swapped, capacity)
...
func TestComputeCexAssetsCommitment_PadsToCapacity(t *testing.T) {
	a := []t1spec.CexAssetInfo{{TotalEquity: 7, BasePrice: 11, Symbol: "X", Index: 0}}
	b := []t1spec.CexAssetInfo{
	...
	const capacity = 3
	ha := t1host.ComputeCexAssetsCommitment(a, capacity)
	hb := t1host.ComputeCexAssetsCommitment(b, capacity)
...
func TestComputeCexAssetsCommitment_RejectsOverCapacity(t *testing.T) {
	...
	too_long := make([]t1spec.CexAssetInfo, 5)
	t1host.ComputeCexAssetsCommitment(too_long, 3)
}
...
func TestAccountLeafHash_Deterministic(t *testing.T) {
	...
	h1 := t1host.AccountLeafHash(account, tiers)
	h2 := t1host.AccountLeafHash(account, tiers)
	if !bytes.Equal(h1, h2) {
		t.Fatalf("not deterministic: %x vs %x", h1, h2)
	}
	...
	h3 := t1host.AccountLeafHash(&account2, tiers)
	...
	account3 := *account
	assets := make([]t1spec.AccountAsset, len(account.Assets))
	copy(assets, account.Assets)
	assets[0].Equity = 200
	account3.Assets = assets
	h4 := t1host.AccountLeafHash(&account3, tiers)
...
func TestPaddingAccounts_RoundsUpToBatch(t *testing.T) {
	...
	newStart, padded := t1host.PaddingAccounts(original, assetKey, paddingStartIndex, usersPerBatch)
	if len(padded) != 4 {
