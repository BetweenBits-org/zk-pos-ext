...
func stubFactory(vfs.Opener, string, int, corespec.PriceScaleProvider) t1spec.SnapshotSource {
	return nil
}
...
func TestSnapshotRegistry_RegisterAndLookup(t *testing.T) {
	...
	if got := t1host.NewSnapshot(id, osvfs.Dir("/tmp"), "snap", 5, nil); got != nil {
...
func TestSnapshotRegistry_NewSnapshotPanicsOnMissing(t *testing.T) {
	...
	t1host.NewSnapshot("never_registered.v0", osvfs.Dir("/tmp"), "snap", 5, nil)
}
