...
func TestAccountLeafHash_LegacyParity(t *testing.T) {
	specAccount := &t4spec.AccountInfo{
		AccountIndex:    7,
	...
	got := t4host.AccountLeafHash(specAccount, assetCountTiers)
...
func TestPaddingAccounts_LegacyParity(t *testing.T) {
	...
	specAccounts := []t4spec.AccountInfo{
		{
			AccountIndex:    1,
			AccountID:       []byte{0xa},
			...
			Assets:          []t4spec.AccountAsset{{Index: 0, Equity: 10}},
		},
		{
			AccountIndex:    2,
			AccountID:       []byte{0xb},
			...
			Assets:          []t4spec.AccountAsset{{Index: 3, Equity: 20, Loan: 5}},
		},
	}
	...
	hostNext, hostPadded := t4host.PaddingAccounts(specAccounts, assetKey, paddingStartFrom, usersPerBatch)
...
func asLegacyAssets(in []t4spec.AccountAsset) []legacyutils.AccountAsset {
