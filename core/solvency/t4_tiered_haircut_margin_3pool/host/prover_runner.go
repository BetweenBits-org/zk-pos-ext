...
// DecodeAndProve decodes the model-typed BatchCreateUserWitness from
// its base64-encoded persistence form, drives groth16.Prove +
// groth16.Verify against the supplied r1cs/pk/vk, and returns the
// universal BatchProofResult cmd/prover persists.
//
// The caller is responsible for snark-param lifecycle (cmd/prover's
// lazy tier cache); this runner only consumes the loaded artifacts.
//
// proof.WriteRawTo emits the uncompressed serialization — same wire
// shape the legacy prover used.
func DecodeAndProve(
	encodedWitness []byte,
	...
	circuitWitness, err := t4circuit.SetBatchCreateUserCircuitWitness(witnessForCircuit, assetCountTiers)
	if err != nil {
