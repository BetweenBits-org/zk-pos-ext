...
type SnapshotSource interface {
	...
	// risk policy (per-asset, per-pool tier curves), computed from the
	// real per-asset tiers before capacity/tier padding — the
	// authorization counterpart to the publicly committed policy. An
	// operator pins this value in profile.toml; a caller checks it via
