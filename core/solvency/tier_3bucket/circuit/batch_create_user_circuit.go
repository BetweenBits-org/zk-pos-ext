...
// Define emits the tier_3bucket constraint set:
//
...
// same length.
func copyTierRatios(dst []TierRatio, src []tier3spec.TierRatio) {
	for i := range dst {
...
func SetBatchCreateUserCircuitWitness(
	batchWitness *tier3spec.BatchCreateUserWitness,
	assetCountTiers []int,
	...
	targetCounts := tier3spec.PickAssetCountTier(
		tier3spec.CountNonEmptyAssets(batchWitness.CreateUserOps[0].Assets),
		assetCountTiers,
	)
	for i := range w.CreateUserOps {
		...
		for j := range batchWitness.CreateUserOps[i].Assets {
			...
			if !tier3spec.IsAccountAssetEmpty(&u) {
