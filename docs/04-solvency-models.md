...
5. **R5 patrón 보존**: `profile/sea_reference` 가 `debt=0` 명시 supply 로 T1 사용 (이전엔 spot_simple).
...
| sea_reference snapshot adapter — `Debt = 0` 명시 supply | `profile/sea_reference/snapshot.go` |
...
# 10. Implementation status (R6 + T2/T3 close)

| Model | spec | circuit | host | setup smoke baseline |
|---|:---:|:---:|:---:|---|
| T1 `t1_simple_margin` | ✅ | ✅ | ✅ | pending (R6.5 env fix) |
| T2 `t2_static_haircut_margin` | ✅ | ✅ | ✅ | pending (R6.5 env fix) |
| T3 `t3_tiered_haircut_margin_1pool` | ✅ | ✅ | ✅ | pending (R6.5 env fix) |
| T4 `t4_tiered_haircut_margin_3pool` | ✅ | ✅ | ✅ | done (R3 step 0, 723790 constraints at tiny shape) |
...
- **T2 implementation (R6/T2)**: T3 의 tier 곡선을 single Haircut basis-points 상수로 collapse. per-asset 1 multiply + 1 division. assetHaircutTable lookup 추가. T3 의 tier-table lookup 제거 → 회로 ~30-40% 절감.

# 11. Open work — R6 follow-up

- **bw6 env fix (R6.5)** — `bnb-chain/gnark-crypto` fork 의 bw6 패키지 부재로 setup smoke / host_test / circuit test 실 실행 불가. fix 후 4 model 모두 R1CS sha256 baseline + NbConstraints 기록.
