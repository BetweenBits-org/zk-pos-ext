...
2. **Per-user solvency** — 모델별로 정의됨. `t4_tiered_haircut_margin_3pool`은 tier-haircut
   기반 사용자별 over-collateral; `t1_simple_margin`은 자동 만족 (부채 없음).
...
제약을 얹는다 (예: t4_tiered_haircut_margin_3pool의 tier-haircut, 3-bucket collateral). `k = None`
...
- **우선 대상**:
  - `t4_tiered_haircut_margin_3pool` 모델 회로 이식 완료 (legacy `circuit/` → `zkpor/core/
    solvency/t4_tiered_haircut_margin_3pool/circuit/`).
  - `binance` 고객사 프로파일 end-to-end 동작 (snapshot CSV loader 포함).
  - 4개 서비스 (witness / prover / userproof / verifier) 가 신규 어댑터 기반
    으로 작동.
- **scope 밖 / 나중 module**:
  - 나머지 3개 모델(`t1_simple_margin`, `t2_static_haircut_margin`,
    `t3_tiered_haircut_margin_1pool`) 회로 구현 — 카탈로그 등재만 유지, 회로는 고객 신호 시 구현.
...
| 목표 고객 (2차) | 한국·EU·일본 규제 spot 거래소 — t1_simple_margin 모델로. |
...
R6 에서 5-tier 가 4-tier 로 통합됨 (spot_simple 흡수, 자세한 트레일은
`docs/04-solvency-models.md §8.1`). marketing 표현은 5-tier 분리 유지:

| Tier (marketing) | Model ID | 타겟 고객 | 통합 난이도 |
|---|---|---|---|
| Basic | `t1_simple_margin` (debt=0) | 한국/EU/일본 regulated spot 거래소, 스테이블코인 발행처, 커스터디 | 가장 낮음 |
| Standard | `t1_simple_margin` | Bybit / KuCoin / HTX 등 mid-tier 마진 거래소 — 기존 Merkle-PoR 의 zk 격상 | 중간 |
| Pro-A | `t2_static_haircut_margin` | 단순 마진 거래소 (단일 collateral pool, 자산-level 고정 haircut) | 중상 |
| Pro-B | `t3_tiered_haircut_margin_1pool` | 파생-heavy 거래소 (size-tiered haircut, 비즈니스 라인 미분리) | 중상 |
| Enterprise | `t4_tiered_haircut_margin_3pool` | Binance / OKX class (VIP loan + cross margin + portfolio margin) | 가장 높음 |

Basic 과 Standard 는 같은 회로 `.vk` 를 공유한다 (G12 closed,
`docs/02-module-architecture.md §6.1`). 이 매핑이 product line 의
정체성이며 GTM 순서의 근거다 (`## Market Context` 참조).
...
1. **t4_tiered_haircut_margin_3pool 시장은 작다** — 채택 가능 거래소 수 기준 2~3개. 단가는
   높지만 deal cycle이 길다.
2. **t1_simple_margin (Standard) / t2_static_haircut_margin 잠재 시장이 가장
   크다** — 마진 사업을 하지만 아직 zk를 안 쓴 거래소 5~10개. zk 격상 욕구가
   있는 mid-tier (Bybit/KuCoin/HTX class).
3. **t1_simple_margin (Basic, debt=0) 도 시장이 넓다** — 한국·EU·일본
   regulated spot 거래소들. 통합 난이도 가장 낮음. 빠른 PMF 검증에 유리.
   같은 ceremony 의 .vk 를 Standard 와 공유.

→ GTM 순서: **Basic 또는 Standard 부터 진입 → Pro로 확장 → Enterprise로
upsell**. t4_tiered_haircut_margin_3pool을 "주력"으로 보지 않는다. t4_tiered_haircut_margin_3pool은 reference
...
|---|---|---:|---|---|
| 🇮🇩 Indonesia | OJK (Bappebti 이관) | 29 (Indodax, Tokocrypto, Pintu 등) | spot 중심, derivatives 신규 framework | `t1_simple_margin` |
| 🇹🇭 Thailand | SEC | ~6 (Bitkub 등) | leverage 0 (1:1 강제), 2026/27 H2 derivatives 도입 예정 | `t1_simple_margin` |
| 🇵🇭 Philippines | BSP (VASP) + SEC (CASP) | 9 (PDAX, Coins.ph 등) | spot only, payments 통합 | `t1_simple_margin` |
| 🇲🇾 Malaysia | SC (RMO-DAX) | 6 (Luno, KDX 등) | spot only | `t1_simple_margin` |
| 🇸🇬 Singapore | MAS (DPT/MPI) | 38 | spot 중심 (리테일 lending/staking 금지), institutional 마진 일부 | `t1_simple_margin` 대부분 / `t1_simple_margin` 일부 |
| 🇻🇳 Vietnam | MoF (2026-01-) | 0 (라이센싱 시작 단계) | TBD ($400M charter capital 요건) | `t1_simple_margin` (예상) |
...
기존 implementation 순서 (R1-R3 = `t4_tiered_haircut_margin_3pool` 우선) 는 **Binance OSS
audit reuse** 라는 engineering 이점에서 출발. 그러나 SEA GTM 기준으로 보면:

| zkpor catalog 모델 | 회로 상태 (2026-05) | SEA 시장 수요 |
|---|---|---|
| `t4_tiered_haircut_margin_3pool` | ✅ 완전 구현 (R3 까지) | ❌ SEA 에 거의 0 |
| `t1_simple_margin` (Basic, debt=0) | ✅ 구현 (R6, spot 흡수) | ✅ **SEA 의 80~100%** |
| `t1_simple_margin` (Standard, margin) | ✅ 구현 (R6) | ⚠ 일부 (Singapore institutional) |
| `t2_static_haircut_margin` | ✅ 구현 (R6 follow-up) | ⚠ 매우 rare |
| `t3_tiered_haircut_margin_1pool` | ✅ 구현 (R6 follow-up) | ❌ SEA 무관 |

→ **현 진척과 시장 수요가 inverted**. R3 step 4 이후의 우선 작업은 SEA GTM
관점에서 **`t1_simple_margin` 회로 구현 (`PRODUCTION_ROADMAP.md` R4 로 재정의)**.
`t4_tiered_haircut_margin_3pool` 의 R3 완성도는 architecture 검증 / audit 신뢰성 / Binance 향
reserve option 으로 가치 유지, 그러나 "주력 제품" 은 t1_simple_margin.
...
| Account leaf | **R6 freeze: 4 model 모두 동일** `Poseidon(AccountID, TotalEquity, TotalDebt, TotalCollateral, AssetsCommitment)`. T1/T2/T3 은 일부 slot 0 supply. 자세한 내용 `docs/04-solvency-models.md §3`. |
| ValueScale | `PriceMultiplier × BalanceMultiplier == ValueScale`. 기본 1e16. |
| Catalog | **R6 freeze: 4-tier circuit (T1~T4)**, 5-tier marketing (Basic = T1 debt=0, Standard = T1 margin). **single selection** per circuit instance, composition 미지원 (v1). |
...
| ID | 질문 | 정해야 하는 시점 |
|---|---|---|
| Q1 | `t4_tiered_haircut_margin_3pool` 회로 이식 후 `.pk`/`.vk` byte-equivalence를 어떤 방법으로 검증? (해시 비교? deterministic re-run?) | R1 진입 전 |
