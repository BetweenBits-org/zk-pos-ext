...
- **Pending (docker-gated)**: live MySQL attest-store ops + e2e smoke
  (attest → verify-user → audit over sample T1 data on docker MySQL). The zk
  smoke is already manual (`scripts/smoke.sh`); the Merkle-sum smoke reuses
  the same MySQL harness with the `attest`/`audit` binaries. Run where docker
  is available before declaring the e2e path verified.
