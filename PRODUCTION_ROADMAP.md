...
- spot_simple 회로 audit 완료 (또는 첫 audit-ready 상태). **CLOSED
...
- **`core/circuit.ComputeFlatUint64Commitment` 잠재 버그 fix (R4-2
  ...
  호출 → panic. tier_3bucket 의 6-field-per-asset 가 3 의 배수라
  안 걸렸음. spot_simple 의 2-field-per-asset 첫 노출. fix 후
  tier_3bucket R1CS sha256 + coefficients sha256 불변 확인 (G1
  baseline 보존).
...
- **noop ConstraintModule promotion** (R3 step 4 의 부수 작업으로
  ...
  의 ConstraintContext field set 이 다르므로 (tier_3bucket 의 collateral/
