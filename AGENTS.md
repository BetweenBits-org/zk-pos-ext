...
이 파일은 이 디렉터리(`zkpor/`)에서 agent가 작업할 때 유지해야 하는 프로젝트
컨텍스트와 구현 원칙을 정의한다. 작업 시작 시 가장 먼저 읽는다.

## Project Context

이 디렉터리(`zkpor/`)는 sibling `../circuit/`, `../src/`(Binance OSS zk-PoR
v2)를 다거래소 N-customer **PoR 엔진 제품**으로 생산화하는 R&D 작업
공간이다. SaaS 형태로 엔진 프로그램을 판매하는 것이 목표이며, 호스팅
인프라·verifier 웹 UI·컴플라이언스 인증은 범위에 포함되지 않는다.
...
- `./` — 신규 modular engine. 모든 신규 작업은 여기.
  - `./core/spec/` — universal interfaces + 5-tier catalog 상수.
  - `./core/circuit/` — universal zk helpers (Merkle, commitment, arith).
  - `./core/solvency/<model>/` — audited math 카탈로그. 5-tier.
  - `./profile/<customer>/` — 고객사 deployment.
  - `./docs/` — methodology 문서 (project-context, production-roadmap).
  - `./HANDOFF.md` — 현재 시점 인수인계.
...
- **raw 입력은 그대로 핵심 경계에 넣지 않는다.** adapter/normalizer가
  canonical 형태로 변환한다. snapshot CSV → AccountInfo는 `./profile/
...
- `../circuit/`, `../src/` legacy 코드는 직접 수정 금지. 변경이 필요하면
  `./` 안에 신규 코드로 추가하고 점진적으로 대체한다.
- `core/solvency/` 추가(=새 모델 카탈로그 등재)는 audit + trusted setup
...
- 고객사 이름을 model identifier에 박지 않는다. `tier_3bucket`, not
  `binance_v2`. 고객사 이름은 `profile/<customer>/` 디렉터리 이름에만.
...
- 새 모델 / 새 customer / 새 ConstraintModule 등 카탈로그·계약 영향이 있는
  결정은 단발 결정으로 두지 말고 `./PRODUCTION_ROADMAP.md`의 gate
  register에 반영한다.
