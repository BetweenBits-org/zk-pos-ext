> ⚠️ **FROZEN — 1차 로드맵 이력 (2026-06-22 동결).** 이 문서는 더 이상 편집하지
> 않는다. go-forward 계획은 `PRODUCTION_ROADMAP.md`(두 트랙: v1 freeze / v2
> sovereign fork)를 본다. 본 문서는 1차 로드맵(Stage R0~MS, closed gates
> G1·G2·G4~G13·G17~G19)의 **결정 트레일 보존용**이다. closed gate 의 근거를
> 추적할 때만 참조한다.

# Production Roadmap — zkpor Engine (v1, FROZEN)
...
### Stage R13 — Prove-path 가속 & 스케일 (GPU 포팅 + multi-instance enablement)

**목적**: prove path 의 wall-clock 을 줄인다 — (1) GPU 가속 backend 를
**포팅**(R&D 아님 — sibling `bb-por` 트랙이 동일 gnark-fork 위에서 Icicle
v3 로 이미 구현·벤치마크 완료: L40S 에서 **2.3× speedup**, MSM 1M-청크
커널 버그 해결), (2) 단일 큐에 여러 prover 를 안전하게 붙일 수 있게
**claim 원자성**만 코어 계약으로 보장한다. 기존 forward stage "R12(GPU)"
\+ "R13(multi-instance)" 를 이 한 stage 로 통합한다.

**범위 한계 (코어 철학)**:
- **코어 (engine)**: GPU backend 포팅 + build-tag on/off seam; claim 원자성
  (adapter correctness). 엔진 본체(`pkg/prover`)는 GPU/병렬에 대해 **분기
  없음** — prove 옵션은 build-tag 로, 배치 정합은 verifier `chainCheck` 가
  이미 보장.
- **외부 (operations)**: prover distribution(Redis BLPOP / SQS / k8s Job),
  worker 수, autoscaling(ASG), queue monitoring. `Scope Boundary` 상 engine
  밖 — `bb-por` 의 ASG 가이드 + Redis 큐가 참고 구현.

**Entry blocker**:
- R11-D CPU baseline 측정 (speedup 검증 대상).
- `bb-por` 의 Icicle v3 backend 번들(`gnark-fork-icicle-v3.bundle`,
  `perf/bb-por-optimizations`) 을 zkpor fork 에 적용 가능한지 확인 — 양쪽
  모두 `bnb-chain/gnark v0.10.x` 베이스(zkpor 는 remote replace, bb-por 는
  로컬 vendor)라 포팅 경로 존재.

**작업 분해**:

- **R13-A** — GPU backend 포팅 (코어)
  - zkpor 의 gnark fork 를 로컬 `replace … => ./gnark-fork` 로 vendor.
  - `bb-por` 의 Icicle v3.2.2 backend (`backend/groth16/bn254/icicle/*` +
    `prove.go` ComputeH export) 적용. **MSM 청크 1M(`1<<20`)** 고정(4M+
    에서 invalid proof 내는 v3.2.2 커널 버그 우회), `RunOnDevice` 채널 동기화.
  - icicle indirect dep v1.1.0/iciclegnark v0.1.0 → `icicle-gnark/v3 v3.2.2`.
- **R13-B** — build-tag on/off seam + runner 배선 (코어)
  - `core/host/prove_icicle.go`(`//go:build icicle` →
    `[]backend.ProverOption{backend.WithIcicleAcceleration()}`) +
    `prove_cpu.go`(`//go:build !icicle` → nil). 4개
    `core/solvency/<model>/host/prover_runner.go` 가
    `groth16.Prove(r1cs, pk, witness, corehost.ProverOptions()...)`.
  - on/off = **빌드 타임**(`-tags icicle`); 런타임 엔진 config 플래그 도입
    **금지**(deploy 관심사를 엔진에 박지 않음 — CPU 빌드는 CGO-free 유지).
  - Make: `build-prover` / `build-prover-gpu`.
- **R13-C** — GPU 측정 smoke
  - g6e.4xlarge(L40S) 또는 g6.4xlarge(L4). production T4 + mid-tier 시나리오.
  - 회로/witness/proof byte-equivalence 보존(GPU/CPU 가 같은 `.vk` 로 verify).
  - 결과 → `docs/reports/`; `bb-por` 2.3× 와 대조.
- **R13-D** — Multi-instance claim-safety (adapter only)
  - `store/witness.go::ClaimOldestByStatus` 에 `SELECT … FOR UPDATE SKIP
    LOCKED`(MySQL 8.0) 또는 조건부 UPDATE(`AND status=from` + RowsAffected)
    추가 → N prover 동시 claim 안전(중복 prove 제거). **엔진 본체 0 변경.**
  - 배치 체인 정합은 verifier `chainCheck` 가 worker 수·순서 무관하게 이미
    보장 → 추가 로직 불요.
  - distribution(Redis 큐/ASG)은 외부 — 필요 시 별 `WitnessQueue` 어댑터
    (cmd 주입). 엔진은 port 계약만 신뢰.

**Exit criteria**:
- GPU 빌드(`-tags icicle`)가 T4 1-batch prove 를 CPU 대비 ≥2× 단축(`bb-por`
  L40S 2.3× 재현 수준), 같은 `.vk` 로 verify ok.
- CPU 빌드(태그 없음)는 CGO-free 로 그대로 빌드·동작(회귀 0).
- claim-safety 후 N(≥2) prover 동시 실행 시 배치 중복 prove 0, chain verify pass.

**관련 Gate**: G15 (`deferred → closed` 후보 — `bb-por` 가 채택 근거 실측
제공). claim-safety 는 계약 freeze 미해당(adapter correctness).

**비-목표**:
- 한 프로세스 내 multi-GPU / GPU pool 공유.
...
- **R13-C** ✅ **GPU prove + verify 성공 + 크로스오버 실측** (DLAMI g6.4xlarge
  L4, CUDA 12.8): `-tags icicle` + `acceleration=icicle` + verify pass. tiny
  174k에선 GPU 0.40×(짐), **4.4M(shape 50_200)에선 GPU 2.65× 전체 / 5.18×
  prover phase**(MSM/NTT 11.16s→2.15s) — 크로스오버는 4.4M 미만, scale 따라
  GPU 이득 증가(bb-por 64M → 2.3×). 절차·결과: `docs/R13_GPU_RUNBOOK.md`.
...
> "R13(multi-instance)" 는 위의 통합 **R13 — Prove-path 가속 & 스케일** 로
> 대체됐다 (2026-06 재구성). GPU 는 sibling `bb-por` 트랙이 이미 구현·벤치마크
> 했으므로 R13 은 *R&D 가 아니라 포팅* 이다.
...
잔여 항목은 R13-D (claim-safety) + 외부 Redis distribution 또는 별도 adapter 슬라이스
로 carry — port 가 이미 그 작업의 seam 을 제공.

### Roadmap path (R10 이후 stages)

```
R10 closed → R11 (test bed) → R13 (Prove-path 가속 & 스케일: GPU 포팅 + claim-safety, G15 closure)
                                ↘
                                  V1-PROD candidate
                                   (first customer + production keygen + first smoke)
```

R11 은 V1-PROD 와 병렬 가능 (서로 의존성 약함). R13 은 GTM SLA
...
**Implementation status (v0.1.1 → v0.1.2)** — landed:

- **S1 (v0.1.1, commit 03c0ca2)** ✅ core primitives — `core/sumtree`
  (dense Poseidon/BN254 sum tree), `core/host.VerifyMerkleSumProof`,
  `core/host` reconcile (`CheckNonNegative`/`NoDuplicate`/`Range`/
  `SumEquality` + `Reconcile`).
- **S4-runner (commit 17b2a96)** ✅ `core/host.AttestStore` port +
  `SumLeafRecord`/`AttestProofDTO`; `t1_simple_margin/host.CollectSumLeaves`
  (Balance = TotalEquity − TotalDebt, dense positional leaves).
- **S2 (commit 8123897)** ✅ `pkg/merklepor` — `RunAttest`/`RunVerifyUser`/
  `RunAudit` + model-blind `buildAndPersist`/`verifyUserConfig` + T1 dispatch.
- **S3 (commit 106dee8)** ✅ `store.AttestationStore` + `AttestStoreAdapter`
  (column `leaf_index` avoids SQL reserved word); `cmd/attest`, `cmd/audit`.
- Verified: `cd zkpor && go build / vet / test -short ./...` green —
  sumtree + merklepor unit tests cover build → persist → verify round trip
  and negative/dup/range/sum-mismatch/tamper rejection. zk PoS line
  import-isolated and unaffected.
- **S4 e2e smoke (commit 5406893)** ✅ VERIFIED on docker MySQL
  (`zkpor-smoke-mysql`, t1_reference happy fixture, 10 accounts): `attest`
  → 10 attestation rows persisted (root `2d690bab…`, total
  1010000000000000) → `verify-user` → sum-inclusion `pass` → `audit` →
  happy `ok`; reserves==liabilities `solvent` exit 0; reserves<liabilities
  `INSOLVENT` exit 1. `cmd/verify-user` + `attest -dump-user-path` added to
  complete the path; `pkg/merklepor` blank-imports the t1_standard_csv.v1
  connector.

**Blocking gate**: G19 (design lock — closed). reserves attestation 모듈 +
salt 정식화는 post-v1 별 gate.
...
| **G5** RiskPolicy 데이터 schema | deferred | R2 | 현재 `cex_assets_info.csv` 형식 (legacy). standard CSV 로 G18 에서 freeze. | CSV 유지 vs JSON/YAML schema 도입 결정. authorization(operator pin) 측면은 **G20** 참조. |
...
| **G19** Merkle-sum PoR 사이드 라인 design lock (D1~D5) | closed | Stage MS | **별도 부속 product line 채택 — 감사인-신뢰 non-zk proof-of-liabilities.** D1 **(A) Merkle Sum Tree** (노드가 자식 sum carry → root 가 총부채; 사용자 sum 기여 공개검증. plain SMT+off-tree 합산 기각). D2 **분리** — 별 `pkg/merklepor`, 메인 verifier 무변경 ("메인 약화 금지"). D3 **T1 전용** — 순잔고 정의가 깔끔한 `t1_simple_margin` 만, `t1host.NewSnapshot` 재사용; haircut(T2~T4) 은 zk 라인 책임. D4 **부채까지만** — 엔진은 attested 부채 root+total, reserves(온체인 지갑) 총액은 감사 input → `RunAudit` 이 `Reserves ≥ Liabilities` 비교 (멀티체인 RPC/서명검증 scope 밖). D5 **프라이버시** — leaf identity = 동결 `AccountLeafHash` 재사용 + batch 셔플, per-user salt 는 G2 의 V2(HMAC/salt) 로 deferral. 구조적 격리: `core/circuit`/`pkg/{keygen,prover}`/groth16/`.pk`·`.vk`/ceremony import 의존 0. | **S1~S4 landed + e2e VERIFIED (v0.1.1 → v0.1.2)** — core primitives, `pkg/merklepor`, store adapter, attest/verify-user/audit CLI; build/vet/test -short green; full attest→verify-user→audit e2e passed on docker MySQL (t1_reference happy, solvent/insolvent exit codes). reserves attestation 모듈 + salt 정식화는 post-v1 별 gate. |
| **G20** RiskPolicy authorization (operator-pinned policy commitment) | deferred | 첫 operator-enforced risk-policy customer (B-wiring) | **신뢰 경계 결정 — 정책 author 를 operator 로 고정 (transparency → fail-closed pin).** zk-prover/FE CR (haircut/tier 정책이 tenant CSV 에만 존재 → 거래소가 자기 정책 설정 가능) 검토 결과: **(1) 정책은 이미 public commitment 에 바인딩됨** — T2 `haircut_bp` 는 CEX commitment 에 패킹 (`t2/host/commitment.go:147-150`), T3/T4 tier `(ratio, boundary)` 도 패킹 (`t{3,4}/host/commitment.go convertTierRatiosToBytes`), CEX commitment 은 public `BatchCommitment` 의 입력 (`core/circuit/commitment.go:60`). → 결손은 *투명성* 이 아니라 *operator 권한 고정*. **(2) `precomputed_value` 는 soundness 무관** — 회로가 in-circuit 재유도 (`t{3,4}/circuit/batch_create_user_circuit.go:102` `generateRapidArithmeticForCollateral` + TotalCollateral in-circuit 계산 L193 + leaf 구성 L216 + after-root 일치 강제 L218), 틀린 값은 host leaf≠circuit leaf → late proof 실패지 위조 증명 불가. 유일한 정책-조작 벡터 `(haircut_bp, ratio, boundary)` 는 전부 public-committed → B-pin 으로 닫는 대상. **방향 lock: (A) 정책을 profile 로 이동 = 기각** (정책은 커밋되는 snapshot 데이터; model⟂profile 직교성 + `base_price` 선례 = per-asset 커밋 크기값은 CSV, 전역 스케일만 profile). **(B) operator 가 profile.toml 에 정책 digest pin + 엔진이 snapshot 에서 재계산 후 불일치 reject (fail-closed)** + **(C) core 가 canonical 헬퍼 제공** 하이브리드 채택. operator 콘솔이 정책 author → 1회 수동 입력에서 canonical 정책 데이터 + digest 동시 생성, tenant 는 잔고만. **(C) landed**: `core/tierpolicy` (`BuildTierCurve` = 회로 레시피 동등 precomputed 생성, `PolicyCommitment` = capacity-비의존 Poseidon/BN254 정책 digest), commit f913091, build/vet/test -short green. G5(데이터 schema)·G18(raw schema freeze) 위에 구축. | (1) **parser precomputed 수렴** — documented invariant ("parser validation rejects mismatches", `t3/standard_schema.go:72`) 을 `tierpolicy.BuildTierCurve` 로 실집행 + 현 fixture (`precomputed=0`, recipe-inconsistent) 정정 [landed 675a8ab]. **B-core landed**: T2/T3/T4 SnapshotSource 에 `PolicyCommitment(ctx)` 노출(parse-시점, 패딩 전 real 정책의 capacity-비의존 digest) + `tierpolicy.VerifyCommitment(got, pinHex)` — 외부가 *실제 소비 데이터 ↔ operator pin* 신뢰 대조 가능(drift 0, fail-closed 직전 단계). 남은 **B-wire**: (2) **profile.toml `[risk_policy] commitment`** additive 필드 (G4/G18 additive 규칙 = minor bump; 빈 값 = 현행 동작, T1 무변경). (3) **snapshot-load digest 대조 reject**. (4) **T2 trusted setup 재실행** — 본 분석 중 발견: T2 회로에 `haircut_bp≤10000` cap 부재(T3/T4의 in-circuit `ratio≤100`과 비대칭) → haircut>10000 으로 collateral over-credit = false-solvency 벡터(commit/B-pin 으로만 탐지, 회로 미차단). **cap landed**(add-only soundness, `t2/circuit/batch_create_user_circuit.go`); R1CS 변경 → T2 trusted setup 재실행 + NbConstraints baseline(48,886→49,936, +1,050/+2.15% at 50 assets, 자산 capacity 에 비례) 재기록 필요. B 는 frozen profile schema 변경이므로 첫 operator-enforced 정책 customer 전 close. |
...
G15 --> R13 (Prove-path 가속 & 스케일: GPU 포팅 ICICLE v3). Entry: R11 closure (CPU baseline). de-risked by sibling bb-por (Icicle v3, L40S 2.3× 실측).
R3 step 4 follow-up (Redis BLPOP queue + multi-worker) --> R13-D (claim-safety, 코어) + distribution (외부 ops).
...
G20 --> 첫 operator-enforced risk-policy customer (B-wiring). 방향 lock: A 기각, B+C 하이브리드. C(core/tierpolicy) landed f913091; B(profile pin + reject) 는 frozen schema 변경이라 customer 전 close.
